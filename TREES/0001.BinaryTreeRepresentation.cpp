...
// Your Task : Complete the function void create_tree(node* root0, vector &vec), which takes a root of a Binary tree and vector array vec containing the values of nodes.
...
node*create(int i , vector<int>&vec)
{
    if(i >= vec.size())
        return NULL;
    struct node*temp = newNode(vec[i]);
    temp->left = create((i*2)+1 , vec);
    temp->right = create((i*2)+2 , vec);
    return temp;
}
